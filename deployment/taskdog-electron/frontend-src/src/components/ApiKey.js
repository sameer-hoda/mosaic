...
export function renderApiKey(root, _state) {
  ...
  validateBtn.addEventListener('click', async () => {
    ...
    if (res.ok) {
      ...
      // NOTE: In the packaged SwiftUI shell, the key would be saved to Keychain
      // here and the backend restarted with the new env var. In dev mode,
      // we expect the key to be set in .env already — we just proceed.
