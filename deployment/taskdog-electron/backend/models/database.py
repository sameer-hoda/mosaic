...
def get_whatsapp_db_conn():
    ...
    In dev, the DBs are at ../../whatsapp-mcp/whatsapp-bridge/store/.
    In Electron, the bridge runs from userDataDir and creates store/ there.
    We check both locations.
    """
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))

    # Dev path
    dev_store = os.path.join(base_dir, 'whatsapp-mcp', 'whatsapp-bridge', 'store')

    # Electron path: bridge cwd is userDataDir, store/ is under it
    electron_store = os.path.join(os.environ.get('HOME', ''), 'Library', 'Application Support', 'taskdog', 'store')

    # Also check CONFIG_FILE env var (set by Electron main.js) to derive userData
    config_file = os.environ.get('CONFIG_FILE', '')
    if config_file:
        electron_store = os.path.join(os.path.dirname(config_file), 'store')

    # Pick whichever exists
    store_dir = dev_store
    if not os.path.exists(os.path.join(dev_store, 'messages.db')):
        if os.path.exists(os.path.join(electron_store, 'messages.db')):
            store_dir = electron_store
        else:
            return None
