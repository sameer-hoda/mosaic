    ...
    if _is_port_open(8080):
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                # Must have a JID to be truly connected (not just WebSocket open)
...
def _save_key_to_env(key: str):
    """Persist the Gemini API key. In Electron, the app bundle is read-only,
    so we skip file persistence — the key is saved via IPC to config.json
    by the frontend, and set in os.environ for the current session."""
    try:
        env_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '.env')
        if os.access(os.path.dirname(env_path), os.W_OK):
            lines = []
            found = False
        ...
        print(f"[setup] Could not write .env (expected in Electron): {e}")
